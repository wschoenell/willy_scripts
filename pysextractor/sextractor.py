...
class SExtractor:
    ...
    def read_file(self, filename, cleanconfig=False, updateconfig=True):
        """
        Read configuration from a .sex file.
        If updateconfig is True (default), the configuration
        files will be updated.
        If cleanconfig is True, the configuration will be 
        set to only the config defined on the configfile.
        """
        if cleanconfig:
            self.config = {}
        ...
        for line in f.readlines():
            params = line.split()
            try: #If there is a keyword, store it.
                
                if not params[0].startswith('#'):
                    self.config[params[0]] = params[1]
                    aux_i = 2 # For those parameters which could have multiple numbers separated by commas. (i.e. PHOT_AUTOPARAMS) 
                    while params[aux_i-1].endswith(','):
                        if aux_i == 2: self.config[params[0]] = [params[1]] # Convert string to list.
                        self.config[params[0]].append(params[aux_i]) # Append value to list.
                        aux_i = aux_i +1
                        
            except: #Otherwise, go ahead to the next line...
                pass
        
        self.config['FILTER_MASK'] = self._SE_config['FILTER_MASK']['value']
        self.config['PARAMETERS_LIST'] = self._SE_config['PARAMETERS_LIST']['value']
        self.config['CONFIG_FILE'] = self._SE_config['CONFIG_FILE']['value']
