import pymongo
import sys
import datetime as dt

host = '192.168.20.118'
# host = None

client = pymongo.MongoClient(host=host)

def print_data(query):
    for obj in query:
        if 'OBJECT' in obj.keys():
            if not "FILTER" in obj.keys():
                print('echo "Downloading {OBJECT}. Night {_night}, Exptime = {EXPTIME}"'.format(**obj))
            else:
                print('echo "Downloading {OBJECT}. Night {_night}, Filter = {FILTER}, Exptime = {EXPTIME}"'.format(**obj))
        elif 'IMAGETYP' in obj.keys():
            print('echo "Bias"')
        else:
            print('echo "Night {night}, Filter = {filter}, Exptime = {exptime}"'.format(**obj))
        print(addr_str.format(**obj))

if len(sys.argv) > 2:
    query = client.images.fits_keywords.find({"OBJECT": {"$regex": "^%s.*" % sys.argv[1]}, "FILTER": sys.argv[2]})
    addr_str = 'wget -c --no-check-certificate https://XXXXXX:XXXXXXX@splus.astro.ufsc.br/{_night}/{FILENAME}'
if sys.argv[1] == 'skyflat':
    pipe = [
        {'$match': {"$and": [{"filter": sys.argv[2]},
                             {"avg_counts": {"$gt": 8000}}, {"avg_counts": {"$lt": 45000}},
                            {'night': {'$gt': (dt.datetime.today() - dt.timedelta(days=60)).strftime('%Y%m%d')}}
        ]}},
        # {'$li mit': 11},
        {'$sort': {"night": 1}}
    ]
    print('# Query: '+str(pipe))
    query = client.images.skyflats.aggregate(pipeline=pipe)
    addr_str = 'wget -c --no-check-certificate https://XXXX:XXXXXX@splus.astro.ufsc.br/{night}/{FILENAME}'
elif sys.argv[1] == 'skyflat-date':
    for fid in sys.argv[2].split(','):
        pipe = [
            {'$match': {"$and": [{"filter": fid},
                                 {"avg_counts": {"$gt": 8000}}, {"avg_counts": {"$lt": 45000}},
                                 {'night': {'$lt': (dt.datetime.strptime(sys.argv[3], '%Y-%m-%d') + dt.timedelta(days=10)).strftime('%Y%m%d')}},
                                 {'night': {'$gt': (dt.datetime.strptime(sys.argv[3], '%Y-%m-%d') - dt.timedelta(days=10)).strftime('%Y%m%d')}}
                                 ]}},
            # {'$li mit': 11},
            {'$sort': {"night": 1}}
        ]
        print('# Query: '+str(pipe))
        query = client.images.skyflats.aggregate(pipeline=pipe)
        addr_str = 'wget -c --no-check-certificate https://XXXX:XXXXX@splus.astro.ufsc.br/{night}/{FILENAME}'
        print_data(query)
    sys.exit(0)
elif sys.argv[1] == 'bias-date':
    for night in sys.argv[2].split(','):
        pipe = [
            {'$match': {"$and": [

                                 {'IMAGETYP': "zero"},
                                 {'_night': {'$eq': (dt.datetime.strptime(night, '%Y-%m-%d')).strftime('%Y%m%d')}},

                                 ]}},
            # {'$li mit': 11},
            {'$sort': {"night": 1}}
        ]
        print('# Query: '+str(pipe))
        query = client.images.fits_keywords.aggregate(pipeline=pipe)
        addr_str = 'wget -c --no-check-certificate https://XXXXX:XXXXXXX@splus.astro.ufsc.br/{_night}/{FILENAME}'
        print_data(query)
    sys.exit(0)
else:
    query = client.images.fits_keywords.find({"OBJECT": {"$regex": "^%s.*" % sys.argv[1]}})
    addr_str = 'wget -c --no-check-certificate https://XXXXX:XXXXX@splus.astro.ufsc.br/{_night}/{FILENAME}'


print_data(query)