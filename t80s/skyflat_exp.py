...
for filter_id in sys.argv[-1].split(','):
    for dirname in sys.argv[1:-1]:
        ...
        for file in imglist:
            ...
            image__header = image[0].header
            long = image__header['LONGITUD']
            lat = image__header['LATITUDE']
            elevation = float(image__header['ALTITUDE'])
            date = image__header['DATE-OBS']
            date = string.replace(date, '-', '/')
            date = string.replace(date, 'T', ' ')
            date = string.replace(date, '\'', '')
            exptime = image__header['EXPTIME']
            ...
            if aux_counts < 65000 and image__header['IMAGETYP'].startswith(
